    ...
    return observableCombineLatest(
      ...
      map(([data, sort]) => {
        if (!_.isNil(data) && sort && sort.active) {
          ...
          return _.isEqual(sort.direction, 'asc') ? sortedData : _.isEqual(sort.direction, 'desc')
            ? sortedData.reverse() : !sort.direction ? _.sortBy(data, ['ID']) : data;
        }
        return data;
      }));
  }
